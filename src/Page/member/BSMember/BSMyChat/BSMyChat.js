...
// ---------------------
    ...
    showCase=()=>{
        fetch('http://localhost:3000/chat/bsMyCase_showCase/'+this.sid)
            .then(res=>res.json())
            .then(data=>{     //回傳 [{bs_case_detail+bs_case資訊},{bs_case_detail+bs_case資訊}...]
                //去讀取應徵網紅資料
                ...
                .then(()=>{
                    ...
                    this.CurrentIC_idx = $(`div[sid=${this.bs_case_detail_sid}]`).find($(`button[data-num]`)).attr('data-num');
                })
                .then(()=>{
...
// --------------------顯示對話
    showChat=(evt)=>{
        ...
        this.talk_sid = casesid; //先把"流水號sid"存起來,當傳送完訊息要重整畫面時會用到
...
fixDate=(v)=>{
    ...
    let final = newDate.getFullYear()+'-'+(newDate.getMonth()+1)+'-'+newDate.getDate()+'　'+timeForm(newDate.getHours())+':'+timeForm(newDate.getMinutes())+':'+timeForm(newDate.getSeconds());
    // console.log(final)
    return final
}
                                                ...
                                                <p>
                                                    <span>
                                                        {   
                                                            ...
                                                            this.state.icNameArray[this.CurrentIC_idx]['IC_name']
