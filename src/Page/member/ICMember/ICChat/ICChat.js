    ...
    showChatFunction=()=>{
       //如果沒有this.BScase_sid => 代表是從接按管理進來的
        if(!this.BScase_sid){
            ...
            fetch('http://localhost:3000/chat/icMyCase_showBSChat/'+this.bs_case_detail_sid)
            .then(res=>res.json())
            .then(data=>{
                this.setState({
                    bsChatArray:data,
            ...
            fetch('http://localhost:3000/chat/icMyCase_showICChat/'+this.bs_case_detail_sid)
                .then(res=>res.json())
                .then(data=>{
                    this.setState({
                        icChatArray:data
        ...
        //如果有this.BScase_sid => 代表是點擊此頁的其他案子
            ...
            fetch('http://localhost:3000/chat/icMyCase_catch/'+this.BScase_sid)
                .then(res=>res.json())
                .then(data=>{
                ...
                fetch('http://localhost:3000/chat/icMyCase_showBSChat/'+this.talk_sid)
                    ...
                    //用流水號sid讀取網紅的對話(存在ic_talk資料表)
                    fetch('http://localhost:3000/chat/icMyCase_showICChat/'+this.talk_sid)
                        .then(res=>res.json())
                        .then(data=>{
                            this.setState({
                                icChatArray:data
